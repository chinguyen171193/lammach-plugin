(function (global) {
	...
	class AgentSpritePlayer {
		constructor(element, options) {
			...
			this.element.classList.add('lm-agent-sprite', 'is-loading');
			this.observe();
			this.register();
			this.loadConfig();
		}
		...
		ensureRig() {
			...
			if (!global.LM_AgentRig || typeof global.LM_AgentRig.markup !== 'function') return false;
			this.sprite.innerHTML = global.LM_AgentRig.markup(this.spriteId, this.portraitLoaded ? this.portraitSource() : '');
		...
		ensureThree() {
			...
			if (this.threeFailed || !definition.model || !global.LM_Agent3D || typeof global.LM_Agent3D.create !== 'function') return false;
			this.removeRig();
			this.threeCharacter = global.LM_Agent3D.create(this.sprite, {
				state: this.state,
				stateMap: definition.stateMap,
				states: definition.states,
				startInBindPose: definition.startInBindPose === true && Boolean(global.LM_AI_OFFICE_ADMIN),
				debug: definition.debug === true && Boolean(global.LM_AI_OFFICE_ADMIN),
				...
				appearance: this.spriteId === 'supervisor-ai' ? ((global.LM_AI_OFFICE_ADMIN && global.LM_AI_OFFICE_ADMIN.supervisorAppearance) || (global.LM_AI_OFFICE && global.LM_AI_OFFICE.supervisorAppearance) || null) : null,
		...
		fallbackFromThree(error) {
			...
			if (global.console && typeof global.console.warn === 'function') {
				global.console.warn('LM AI Office: không tải được nhân vật 3D, đã dùng fallback.', error);
			}
		}
	...
	global.LM_AgentSpritePlayer = AgentSpritePlayer;
