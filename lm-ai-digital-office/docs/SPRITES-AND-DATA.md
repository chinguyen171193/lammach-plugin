...
LM Supervisor AI dùng `playback: "three"`. Model và animation được đóng gói trong plugin, không dùng CDN. Renderer dùng Three.js `AnimationMixer`; state machine nằm riêng trong `public/js/agent-3d-state-machine.js` và cross-fade giữa các clip để không gắn logic chuyển động vào giao diện:
...
Vào **LM AI Office → Nhân viên và AI Agent**, bấm **Demo Agent**. Demo chạy: idle 2 giây → working 8 giây (10–80%) → reviewing 4 giây → done 2 giây → idle cho LM Supervisor AI và cập nhật nhật ký ngay trên trang.

## Kết nối dữ liệu thật

Gửi hook `lm_ai_office_event` khi đơn hàng, Gerber, BOM, sản xuất hay QC thay đổi. Frontend gọi REST chỉ để đọc dữ liệu an toàn; không đưa API key, thông tin khách hàng hoặc dữ liệu nhạy cảm vào endpoint công khai.
