(function ($) {
	...
	class PreviewOrbitCamera {
		...
		rotate(theta, phi) { this.theta += theta; this.phi = Math.max(.2, Math.min(Math.PI - .2, this.phi + phi)); this.update(); }
		zoom(amount) { this.radius = Math.max(3.1, Math.min(11, this.radius + amount)); this.update(); }
		reset() { this.radius = 5.8; this.theta = .68; this.phi = 1.12; this.update(); }
	...
	async function openPreview(button) {
		...
		try {
			const [model, source] = await Promise.all([loadFBX(LM_AI_OFFICE_ANIMATION_PREVIEW.modelUrl), loadFBX(button.dataset.animationUrl)]);
	...
	$(document).on('click', '[data-animation-preview-control]', function () { if (!preview) return; const action = this.dataset.animationPreviewControl; if (action === 'rotate-left') preview.orbit.rotate(.35, 0); if (action === 'rotate-right') preview.orbit.rotate(-.35, 0); if (action === 'zoom-in') preview.orbit.zoom(-.65); if (action === 'zoom-out') preview.orbit.zoom(.65); if (action === 'reset') preview.orbit.reset(); });
