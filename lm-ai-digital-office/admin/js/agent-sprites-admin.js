(function (global) {
	...
	function addLog(message, type) {
		const log = document.querySelector('[data-lm-ai-demo-log]');
		if (log) log.textContent = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' — ' + message;
		if (!global.LM_AI_OFFICE_ADMIN || !global.LM_AI_OFFICE_ADMIN.restNonce) return;
		global.fetch(global.LM_AI_OFFICE_ADMIN.restUrl + 'events', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': global.LM_AI_OFFICE_ADMIN.restNonce },
			body: JSON.stringify({ type: type || 'new_gerber', title: 'Demo Agent', department: 'pcb', priority: 'normal', message: message })
		}).catch(() => {});
	}

	function initSprites() {
		if (!global.LM_AgentSpritePlayer || !global.LM_AI_OFFICE_ADMIN) return;
		document.querySelectorAll('[data-agent-sprite]').forEach(element => {
			...
			global.LM_AgentSpritePlayer.create(element, {
				agentId: element.dataset.agentId,
				spriteId: element.dataset.spriteId,
				state: element.dataset.agentState,
				assetBase: global.LM_AI_OFFICE_ADMIN.agentAssetsUrl,
				assetVersion: global.LM_AI_OFFICE_ADMIN.agentAssetsVersion,
				scale: 1
			});
		});
	}
	...
	async function runDemo(button) {
		...
		addLog('LM Supervisor AI đang ở trạng thái ' + stateLabel('idle') + '.', 'new_customer');
		await wait(2000);
		global.updateAgentState(agentId, 'working');
		addLog('LM Supervisor AI bắt đầu điều phối hoạt động.', 'production_started');
		...
		addLog('LM Supervisor AI đang rà soát dashboard.', 'production_completed');
		await wait(4000);
		global.updateAgentState(agentId, 'done');
		global.updateAgentTask(agentId, 'Đã hoàn thành phiên điều phối', 100);
		addLog('LM Supervisor AI đã hoàn thành phiên điều phối.', 'production_completed');
		await wait(2000);
		global.updateAgentState(agentId, 'idle');
		global.updateAgentTask(agentId, 'Đang chờ nhiệm vụ tiếp theo', 0);
		addLog('Demo hoàn tất. LM Supervisor AI quay lại trạng thái chờ.', 'production_completed');
	...
	document.addEventListener('DOMContentLoaded', () => {
		initSprites();
		const button = document.querySelector('[data-lm-ai-demo-agents]');
