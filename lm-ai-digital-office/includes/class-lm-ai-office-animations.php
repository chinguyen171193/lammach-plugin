...
final class LM_AI_Office_Animations {
	const OPTION = 'lm_ai_office_animation_library';
	...
	public static function valid_asset( $attachment_id ) {
		...
		if ( ! $attachment_id || ! $file || ! file_exists( $file ) ) { return new WP_Error( 'lm_ai_office_animation_file', 'Tệp không tồn tại trong Media Library.' ); }
		$extension = strtolower( pathinfo( $file, PATHINFO_EXTENSION ) );
		if ( ! in_array( $extension, array( 'fbx', 'glb', 'gltf' ), true ) ) { return new WP_Error( 'lm_ai_office_animation_format', 'Chỉ hỗ trợ tệp .fbx, .glb hoặc .gltf.' ); }
	...
	public static function add( $employee, $action, $attachment_id ) {
		...
		$owner = LM_AI_Office_Characters::animation_owner( $employee );
		if ( ! $owner || ! isset( self::actions()[ $action ] ) ) { return new WP_Error( 'lm_ai_office_animation_input', 'Nhân vật hoặc hành động không hợp lệ.' ); }
		...
		foreach ( $variants as $variant ) { if ( absint( $variant['attachment_id'] ?? 0 ) === $asset['attachment_id'] ) { return new WP_Error( 'lm_ai_office_animation_duplicate', 'Tệp này đã được gán cho hành động.' ); } }
	...
	public static function remove( $employee, $action, $index ) {
		$employee = sanitize_key( $employee ); $action = strtoupper( sanitize_key( $action ) );
		$owner = LM_AI_Office_Characters::animation_owner( $employee ); $library = self::library(); if ( ! $owner || ! isset( self::actions()[ $action ], $library[ $owner ][ $action ][ $index ] ) ) { return new WP_Error( 'lm_ai_office_animation_input', 'Animation không hợp lệ.' ); }
