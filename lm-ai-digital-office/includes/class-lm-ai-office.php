...
final class LM_AI_Office {
	...
	private function __construct() {
		add_filter( 'upload_mimes', array( 'LM_AI_Office_Animations', 'upload_mimes' ) );
		new LM_AI_Office_Assets();
		new LM_AI_Office_Shortcode();
		new LM_AI_Office_API();
		new LM_AI_Office_Events();
		if ( is_admin() ) {
			new LM_AI_Office_Admin();
		}
	}

	public static function activate() {
		self::create_tables();
		if ( ! get_option( 'lm_ai_office_seeded' ) ) {
			self::seed_demo_data();
		}
		add_option( LM_AI_OFFICE_OPTION, self::default_settings(), '', false );
	}
	...
	public static function tables() {
		global $wpdb;
		return array(
			'departments' => $wpdb->prefix . 'lm_ai_departments',
			'agents'      => $wpdb->prefix . 'lm_ai_agents',
			'tasks'       => $wpdb->prefix . 'lm_ai_tasks',
			'workflows'   => $wpdb->prefix . 'lm_ai_workflows',
			'events'      => $wpdb->prefix . 'lm_ai_events',
			'logs'        => $wpdb->prefix . 'lm_ai_logs',
		);
	}
	...
	public static function default_settings() {
		return array(
			'company_name'       => 'CÔNG TY TNHH THƯƠNG MẠI DỊCH VỤ SẢN XUẤT ĐIỆN TỬ LM',
			'model_name'         => 'LM AI Digital Office',
	...
	public static function settings() {
		return wp_parse_args( get_option( LM_AI_OFFICE_OPTION, array() ), self::default_settings() );
	}
	...
	private static function seed_demo_data() {
		...
		update_option( 'lm_ai_office_seeded', 1, false );
		self::$dataset = null;
	}
	...
	public static function demo_agents() {
		...
		foreach ( array( array( 'LM Supervisor AI', 'ai_center', '#b388ff' ), array( 'Sale AI', 'sales', '#29b6f6' ), array( 'Gerber AI', 'pcb', '#00e5b7' ), array( 'BOM AI', 'warehouse', '#ffb020' ), array( 'Production AI', 'smt', '#42e695' ), array( 'Finance AI', 'finance', '#ffd166' ) ) as $index => $ai ) {
