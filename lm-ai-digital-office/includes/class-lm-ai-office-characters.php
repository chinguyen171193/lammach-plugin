...
final class LM_AI_Office_Characters {
	const OPTION = 'lm_ai_office_character_models';
	...
	public static function definitions() {
		return array(
			...
			'employee_002' => self::definition( 'employee_002', 'Employee 002', 'Chăm sóc khách hàng', 'public/assets/characters/employee_002/employee_002_worker.fbx', 'Tư thế làm việc ổn định', array( 'x' => -1.8, 'y' => 0, 'z' => 0 ), self::FEMALE_PROFILE ),
		);
	}
	...
	public static function model( $employee, $fallback = '' ) {
		...
		$path = $fallback ? LM_AI_OFFICE_DIR . $fallback : '';
		if ( $path && file_exists( $path ) ) {
			return array( 'attachment_id' => 0, 'url' => LM_AI_OFFICE_URL . $fallback, 'format' => strtolower( pathinfo( $path, PATHINFO_EXTENSION ) ), 'label' => basename( $path ), 'source' => 'bundled', 'available' => true );
		}
	...
	public static function valid_model_asset( $attachment_id ) {
		...
		if ( ! $attachment_id || ! $file || ! file_exists( $file ) ) { return new WP_Error( 'lm_ai_office_model_file', 'Tệp model không tồn tại trong Media Library.' ); }
		$format = strtolower( pathinfo( $file, PATHINFO_EXTENSION ) );
		if ( ! in_array( $format, array( 'fbx', 'glb', 'gltf' ), true ) ) { return new WP_Error( 'lm_ai_office_model_format', 'Chỉ hỗ trợ tệp FBX, GLB hoặc GLTF.' ); }
	...
	public static function set_model( $employee, $attachment_id ) {
		$employee = sanitize_key( $employee ); if ( ! self::exists( $employee ) ) { return new WP_Error( 'lm_ai_office_character', 'Không tìm thấy nhân vật.' ); }
