...
class DAT_PCB_Project {
	...
	public function can_access_project( $project_id ) {
		...
		if ( ! $project_id || 'dat_pcb_project' !== get_post_type( $project_id ) ) {
			return false;
		}
	...
	public function create_project( $args ) {
		if ( ! $this->can_manage() ) {
			return new WP_Error( 'dat_pcb_forbidden', 'Không đủ quyền tạo dự án.', array( 'status' => 403 ) );
		}
	...
	public function read_project( $project_id ) {
		if ( ! $this->can_access_project( $project_id ) ) {
			return new WP_Error( 'dat_pcb_forbidden', 'Không đủ quyền đọc dự án.', array( 'status' => 403 ) );
		}
		$json = get_post_meta( $project_id, DAT_PCB_TRACER_META_JSON, true );
	...
	public function save_project( $project_id, $data ) {
		if ( ! $this->can_access_project( $project_id ) ) {
			return new WP_Error( 'dat_pcb_forbidden', 'Không đủ quyền lưu dự án.', array( 'status' => 403 ) );
		}
		$data = $this->sanitize_project_data( $data );
	...
	public function delete_project( $project_id ) {
		if ( ! $this->can_access_project( $project_id ) ) {
			return new WP_Error( 'dat_pcb_forbidden', 'Không đủ quyền xóa dự án.', array( 'status' => 403 ) );
		}
		return (bool) wp_delete_post( absint( $project_id ), true );
	}

	private function save_data( $project_id, $data ) {
		update_post_meta( $project_id, DAT_PCB_TRACER_META_JSON, wp_json_encode( $data ) );
		update_post_meta( $project_id, DAT_PCB_TRACER_META_CODE, $data['project']['code'] );
		update_post_meta( $project_id, DAT_PCB_TRACER_META_TOP_IMAGE, absint( $data['images']['top']['attachment_id'] ) );
		update_post_meta( $project_id, DAT_PCB_TRACER_META_BOTTOM_IMAGE, absint( $data['images']['bottom']['attachment_id'] ) );
	}
