(function (global) {
	...
	EditorCommandExecutor.prototype.addFootprint = function (command, localPins) {
		...
		var layer = global.LMPCBTracerTools.layerForTool('pad_round', side);
		...
		(command.pins || []).forEach(function (pin) {
			...
			var padId = global.LMPCBTracerTools.makeId();
		...
		// Lm nhan ngay tren canh tren cung cua vung bao quanh chan/outline thuc te,
	...
	EditorCommandExecutor.prototype.routeConnect = function (command, localPins, netGroup) {
		...
		var route = global.LMPCBRouteEngine
			? global.LMPCBRouteEngine.route(this.app.state, { a: a, b: b, trackWidth: width, clearance: clearance, viaDiameter: viaDia, viaDrill: viaDrill, netGroup: netGroup })
			: null;
		if (!route) {
		...
		route.vias.forEach(function (via) {
			var id = global.LMPCBTracerTools.makeId();
			objectIds.push(id);
	...
	global.LMPCBEditorCommandExecutor = EditorCommandExecutor;
})(window);
