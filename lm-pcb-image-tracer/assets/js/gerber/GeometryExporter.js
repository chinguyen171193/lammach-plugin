(function (global) {
	...
	function GeometryExporter(project, options) {
		...
		this.layers = global.LMPCBGerberLayerMapper.createLayers();
		this.drillHoles = [];
		this.warnings = [];
	}
	...
	GeometryExporter.prototype.exportTrack = function (obj) {
		var g = obj.geometry || {};
		var layerId = global.LMPCBGerberLayerMapper.copperLayerForObject(obj);
		var layer = this.layers[layerId === 'bottomCopper' ? 'bottomCopper' : 'topCopper'];
		var width = Number(g.width || 0.4);
		if (Number(g.bow || 0) && global.LMPCBTracerTools && global.LMPCBTracerTools.sampleArcPoints) {
			var pts = global.LMPCBTracerTools.sampleArcPoints(g, 24);
			if (pts && pts.length > 1) {
	...
	GeometryExporter.prototype.exportPad = function (obj) {
		...
		global.LMPCBGerberLayerMapper.maskLayersForObject(obj).forEach(function (maskId) {
	...
	GeometryExporter.prototype.exportAnnotation = function (obj) {
		var g = obj.geometry || {};
		var layer = this.layers[global.LMPCBGerberLayerMapper.silkLayerForObject(obj)];
	...
	global.LMPCBGeometryExporter = GeometryExporter;
})(window);
