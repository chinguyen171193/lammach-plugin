(function (global) {
	...
	function GerberExporter(options) {
		...
		this.formatter = new global.LMPCBGerberCoordinateFormatter({ integerDigits: 4, decimalDigits: 6 });
		this.gerberWriter = new global.LMPCBGerberWriter({ formatter: this.formatter });
		this.excellonWriter = new global.LMPCBExcellonWriter({ formatter: this.formatter });
		this.zip = new global.LMPCBGerberZip();
	}

	GerberExporter.prototype.exportProject = function (project) {
		var clean = this.clone(project);
		var geometry = new global.LMPCBGeometryExporter(clean).export();
		var files = [];
	...
	GerberExporter.prototype.report = function (project, warnings, files) {
		var lines = [
			'LM PCB Image Tracer Gerber Export',
	...
	global.LMPCBGerberExporter = GerberExporter;
})(window);
