(function (global, document) {
	...
	PropertiesPanel.prototype.render = function (panel) {
		panel.textContent = '';
		panel.classList.add('lm-easy-properties');
		var selected = this.app.selectedObjects();
		var obj = selected[0];
		if (!obj) {
			panel.appendChild(el('div', 'lm-easy-empty', 'Chưa chọn đối tượng'));
			return;
		}
	...
	PropertiesPanel.prototype.renderComponent = function (panel, component, pin) {
		...
		if (pin) {
			...
			var button = el('button', 'lm-easy-secondary-button', 'Xem linh kiện');
			button.type = 'button';
	...
	PropertiesPanel.prototype.group = function (panel, title, fields) {
		var group = el('section', 'lm-easy-prop-group');
		var header = el('button', 'lm-easy-prop-group-header');
		header.type = 'button';
		header.textContent = title;
		var body = el('div', 'lm-easy-prop-group-body');
	...
	PropertiesPanel.prototype.field = function (labelText, key, value, target, type, unit, options) {
		var row = el('label', 'lm-easy-prop-row');
		row.appendChild(el('span', 'lm-easy-prop-label', labelText));
		var control = el('span', 'lm-easy-prop-control');
		var input;
		if (type === 'select') {
		...
		if (unit) control.appendChild(el('span', 'lm-easy-unit', unit));
		row.appendChild(control);
		return row;
	};

	PropertiesPanel.prototype.readonly = function (labelText, value) {
		var row = el('div', 'lm-easy-prop-row is-readonly');
		row.appendChild(el('span', 'lm-easy-prop-label', labelText));
		row.appendChild(el('span', 'lm-easy-prop-value', value === undefined || value === '' ? '-' : String(value)));
		return row;
	};
	...
	global.LMPCBPropertiesPanel = PropertiesPanel;
})(window, document);
