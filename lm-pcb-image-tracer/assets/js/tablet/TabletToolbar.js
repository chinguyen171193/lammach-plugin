(function (global, document) {
	...
	function TabletToolbar(app) {
		this.app = app;
		this.root = app.root;
		this.sidebar = app.root.querySelector('.lm-pcb-layers');
		this.rightPanel = app.root.querySelector('.lm-pcb-properties');
	...
	TabletToolbar.prototype.buildCompactPalette = function () {
		...
		palette.className = 'lm-tablet-compact-palette';
		palette.setAttribute('aria-label', this.t('title'));
		var handle = document.createElement('div');
		handle.className = 'lm-tablet-palette-handle';
		var grip = document.createElement('span');
		grip.className = 'lm-tablet-palette-grip';
		grip.textContent = '⠿';
		var title = document.createElement('strong');
		title.textContent = this.t('title');
		var settings = document.createElement('button');
		settings.type = 'button';
		settings.className = 'lm-tablet-palette-settings-button';
		...
		toggle.type = 'button';
		toggle.className = 'lm-tablet-palette-toggle';
		...
		tools.className = 'lm-tablet-compact-tools';
		var config = document.createElement('div');
		config.className = 'lm-tablet-palette-config';
		config.hidden = true;
	...
	TabletToolbar.prototype.renderPaletteConfig = function () {
		...
		heading.className = 'lm-tablet-palette-config-title';
	...
	TabletToolbar.prototype.updateLanguageText = function () {
		...
		var menu = this.root.querySelector('.lm-tablet-overflow-menu');
		if (menu) {
		...
		if (this.collapseButton) {
			var collapsed = this.sidebar && this.sidebar.classList.contains('lm-tablet-icon-only');
	...
	TabletToolbar.prototype.setRightPanelCollapsed = function (collapsed) {
		...
		this.rightPanel.classList.toggle('lm-tablet-panel-collapsed', !!collapsed);
		this.root.classList.toggle('lm-tablet-right-collapsed', !!collapsed);
		var button = this.rightPanel.querySelector('.lm-tablet-right-collapse');
		if (button) {
	...
	TabletToolbar.prototype.setLeftOverlayOpen = function (open) {
		...
		this.root.classList.toggle('lm-tablet-left-overlay-open', !!open);
		this.root.classList.add('lm-tablet-left-collapsed');
		this.sidebar.classList.toggle('lm-tablet-icon-only', !open);
		if (this.collapseButton) {
	...
	TabletToolbar.prototype.initializeResponsiveSidebar = function () {
		...
		if (this.isResponsiveTablet()) {
			...
			this.root.classList.remove('lm-tablet-left-overlay-open');
		}
	};

	TabletToolbar.prototype.buildTopOverflow = function () {
		var toolbar = this.root.querySelector('.lm-pcb-toolbar');
		if (!toolbar) return;
		this.buildLanguageSwitcher(toolbar);
		if (toolbar.querySelector('.lm-tablet-overflow-button')) return;
		var button = document.createElement('button');
		button.type = 'button';
		button.className = 'lm-tablet-overflow-button';
		button.textContent = this.t('more');
		this.overflowButton = button;
		var menu = document.createElement('div');
		menu.className = 'lm-tablet-overflow-menu';
		menu.hidden = true;
		[
			['new', 'new', 'action'],
			['open', 'open', 'action'],
	...
	TabletToolbar.prototype.buildLanguageSwitcher = function (toolbar) {
		if (toolbar.querySelector('.lm-tablet-language-switcher')) return;
		var wrap = document.createElement('label');
		wrap.className = 'lm-tablet-language-switcher';
	...
	TabletToolbar.prototype.buildLeftPanel = function () {
		if (!this.sidebar || this.sidebar.querySelector('.lm-tablet-left-shell')) return;
		var shell = document.createElement('div');
		shell.className = 'lm-tablet-left-shell';
		this.leftShell = shell;
		var header = document.createElement('div');
		header.className = 'lm-tablet-panel-header';
		...
		collapse.className = 'lm-tablet-collapse';
		...
		searchWrap.className = 'lm-tablet-tool-search';
		var searchIcon = document.createElement('span');
		searchIcon.className = 'lm-tablet-search-icon';
		...
		body.className = 'lm-tablet-left-body';
		shell.appendChild(body);
		this.leftBody = body;
		var settings = document.createElement('div');
		settings.className = 'lm-tablet-tool-settings';
		...
		collapse.addEventListener('click', function () {
			if (this.isResponsiveTablet()) {
				this.setLeftOverlayOpen(!this.root.classList.contains('lm-tablet-left-overlay-open'));
				return;
			}
			this.sidebar.classList.toggle('lm-tablet-icon-only');
			this.root.classList.toggle('lm-tablet-left-collapsed', this.sidebar.classList.contains('lm-tablet-icon-only'));
			collapse.innerHTML = this.renderIcon(this.sidebar.classList.contains('lm-tablet-icon-only') ? 'panel-left-open' : 'panel-left-close');
			collapse.title = this.sidebar.classList.contains('lm-tablet-icon-only') ? this.t('expand') : this.t('collapse');
			collapse.setAttribute('aria-label', this.sidebar.classList.contains('lm-tablet-icon-only') ? this.t('expand') : this.t('collapse'));
		}.bind(this));
		document.addEventListener('pointerdown', function (e) {
			if (!this.isResponsiveTablet() || !this.root.classList.contains('lm-tablet-left-overlay-open')) return;
		...
		this.canvas.addEventListener('pointerdown', function () {
			if (this.isResponsiveTablet() && this.root.classList.contains('lm-tablet-left-overlay-open')) {
				this.setLeftOverlayOpen(false);
			}
		}.bind(this));
	...
	TabletToolbar.prototype.renderIcon = function (name) {
		...
		return '<svg class="lm-tablet-lucide" viewBox="0 0 24 24" aria-hidden="true" focusable="false">' + (paths[name] || paths.circle) + '</svg>';
	};

	TabletToolbar.prototype.createToolGroup = function (category, tools) {
		var section = document.createElement('section');
		section.className = 'lm-tablet-tool-group';
		...
		summary.type = 'button';
		summary.className = 'lm-tablet-category-button';
		summary.setAttribute('data-sidebar-category', category.id);
		summary.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
		summary.innerHTML = '<span class="lm-tablet-category-icon">' + this.renderIcon(category.icon) + '</span><span class="lm-tablet-category-label">' + this.getCategoryLabel(category) + '</span><span class="lm-tablet-category-count">' + tools.length + '</span>';
		section.appendChild(summary);
		var grid = document.createElement('div');
		grid.className = 'lm-tablet-tool-grid';
		if (tools.length) {
			...
			empty.className = 'lm-tablet-empty-tools';
	...
	TabletToolbar.prototype.createToolCard = function (tool) {
		...
		card.className = 'lm-tablet-tool-card';
		...
		card.innerHTML =
			'<span class="lm-tablet-tool-icon">' + this.renderIcon(info.icon) + '</span>' +
			'<span class="lm-tablet-tool-label">' + this.getToolLabel(tool) + '</span>' +
			'<span class="lm-tablet-tool-shortcut">' + (info.shortcut || '') + '</span>';
		card.addEventListener('dragstart', function (e) {
			if (info.action) {
				e.preventDefault();
				return;
			}
			e.dataTransfer.setData('text/lm-pcb-tool', info.tool || tool);
	...
	TabletToolbar.prototype.openCategory = function (categoryId) {
		...
		if (this.sidebar && this.sidebar.classList.contains('lm-tablet-icon-only')) {
			this.sidebar.classList.remove('lm-tablet-icon-only');
			this.root.classList.remove('lm-tablet-left-collapsed');
			var collapse = this.sidebar.querySelector('.lm-tablet-collapse');
			if (collapse) {
				collapse.innerHTML = this.renderIcon('panel-left-close');
				collapse.title = this.t('collapse');
				collapse.setAttribute('aria-label', this.t('collapse'));
	...
	TabletToolbar.prototype.createSettingField = function (label, input) {
		...
		row.className = 'lm-tablet-tool-setting-row';
	...
	TabletToolbar.prototype.createRouteModeSetting = function () {
		...
		wrap.className = 'lm-tablet-tool-setting-row lm-tablet-route-mode-row';
		var label = document.createElement('span');
		label.textContent = this.t('routeMode');
		var group = document.createElement('div');
		group.className = 'lm-tablet-route-mode';
	...
	TabletToolbar.prototype.renderToolSettings = function () {
		...
		title.className = 'lm-tablet-tool-settings-title';
		title.innerHTML = '<span>' + this.t('toolSettings') + '</span><strong>' + this.getToolLabel(tool) + '</strong>';
		var body = document.createElement('div');
		body.className = 'lm-tablet-tool-settings-body';
		...
		if (!body.childNodes.length) {
			var empty = document.createElement('div');
			empty.className = 'lm-tablet-tool-settings-empty';
	...
	TabletToolbar.prototype.buildRightTabs = function () {
		if (!this.rightPanel || this.rightPanel.querySelector('.lm-tablet-right-tabs')) return;
		var original = Array.prototype.slice.call(this.rightPanel.childNodes);
		var tabs = document.createElement('div');
		tabs.className = 'lm-tablet-right-tabs';
		var collapse = document.createElement('button');
		collapse.type = 'button';
		collapse.className = 'lm-tablet-right-collapse';
		...
		var body = document.createElement('div');
		body.className = 'lm-tablet-right-body';
		...
		tabs.addEventListener('click', function (e) {
			var collapseButton = e.target.closest('.lm-tablet-right-collapse');
			if (collapseButton) {
				this.setRightPanelCollapsed(!this.rightPanel.classList.contains('lm-tablet-panel-collapsed'));
				return;
			}
			var tab = e.target.getAttribute('data-tablet-tab');
			if (!tab) return;
			if (tab === 'ai' && this.app.root.querySelector('.lm-tablet-ai-panel')) {
				var aiButton = this.app.root.querySelector('.lm-tablet-ai-button');
	...
	TabletToolbar.prototype.createTabPane = function (name) {
		...
		pane.className = 'lm-tablet-tab-pane';
	...
	TabletToolbar.prototype.createInfoPane = function (name, text) {
		...
		data.className = 'lm-tablet-tab-data';
	...
	TabletToolbar.prototype.refreshInfoPanes = function () {
		if (this.historyPane) {
			var h = this.historyPane.querySelector('.lm-tablet-tab-data');
		...
		if (this.bomPane) {
			...
			var b = this.bomPane.querySelector('.lm-tablet-tab-data');
	...
	TabletToolbar.prototype.bindCanvasDrop = function () {
		this.canvas.addEventListener('dragover', function (e) {
			...
			if (types.indexOf('text/lm-pcb-tool') !== -1) e.preventDefault();
		});
		this.canvas.addEventListener('drop', function (e) {
			var tool = e.dataTransfer.getData('text/lm-pcb-tool');
			if (!tool) return;
			e.preventDefault();
			this.app.setTool(tool);
	...
	TabletToolbar.prototype.enhanceBottomBar = function () {
		global.setTimeout(function () {
			var fab = this.root.querySelector('.lm-tablet-fab');
	...
	global.LMPCBTabletToolbar = TabletToolbar;
})(window, document);
