(function (global, document) {
	...
	function applyFrontendFullscreen(root) {
		if (!root.classList.contains('lm-pcb-tracer-fullscreen')) return;
		var wrap = root.closest('.lm-pcb-tracer-shortcode-wrap') || root.parentElement;
		document.documentElement.classList.add('lm-pcb-tracer-fullscreen-active');
		document.body.classList.add('lm-pcb-tracer-fullscreen-active');
		if (wrap) {
	...
	function App(root) {
		...
		this.projectId = Number(global.LMPCBTracer.projectId || 0);
		this.tool = 'select';
		this.activeSide = 'top';
		...
		this.history = new global.LMPCBTracerHistory(100);
		this.dirty = false;
		this.drawing = null;
		this.calibration = null;
		this.measure = null;
		this.clipboard = [];
		this.canvas = new global.LMPCBTracerCanvas(qs(root, '#lm-pcb-tracer-canvas'), this);
		this.layersPanel = global.LMPCBLayersPanel ? new global.LMPCBLayersPanel(this) : null;
		this.propertiesPanel = global.LMPCBPropertiesPanel ? new global.LMPCBPropertiesPanel(this) : null;
		this.personalLibrary = global.LMPCBPersonalLibrary ? new global.LMPCBPersonalLibrary(this) : null;
		this.buildFileMenu();
		this.bind();
	...
	App.prototype.buildFileMenu = function () {
		var toolbar = qs(this.root, '.lm-pcb-toolbar');
		if (!toolbar || qs(toolbar, '.lm-pcb-file-menu')) return;
		var self = this;
		var menu = el('div');
		menu.className = 'lm-pcb-file-menu';
		var trigger = el('button', 'File');
		trigger.type = 'button';
		trigger.className = 'lm-pcb-file-menu-button';
		trigger.setAttribute('aria-haspopup', 'true');
		trigger.setAttribute('aria-expanded', 'false');
		var popover = el('div');
		popover.className = 'lm-pcb-file-menu-popover';
		popover.hidden = true;
		var exportLabel = el('div', 'Xuất');
		exportLabel.className = 'lm-pcb-file-menu-label';
		var gerber = el('button', 'Gerber (.zip)');
		gerber.type = 'button';
		gerber.setAttribute('data-action', 'export-gerber');
		var libraryLabel = el('div', 'Thư viện');
		libraryLabel.className = 'lm-pcb-file-menu-label';
	...
	App.prototype.loadInitial = function () {
		var self = this;
		if (this.projectId) {
			global.LMPCBTracerStorage.read(this.projectId).then(function (res) {
				self.state = res.data;
				self.normalizeState();
				self.activePinId = '';
				self.renderAll();
	...
	App.prototype.createTrackSegment = function (start, end) {
		...
		return {
			id: global.LMPCBTracerTools.makeId(),
			type: 'track',
	...
	App.prototype.createNew = function () {
		...
		global.LMPCBTracerStorage.create({ name: data.project.name, code: data.project.code, width_mm: data.board.width_mm, height_mm: data.board.height_mm }).then(function (res) {
	...
	App.prototype.loadProject = function (id) {
		var self = this;
		this.projectId = id;
		return global.LMPCBTracerStorage.read(id).then(function (res) {
			self.state = res.data;
			self.normalizeState();
			self.selected = [];
			self.activePinId = '';
	...
	App.prototype.pointerDown = function (p, event) {
		...
		var obj = global.LMPCBTracerTools.objectFromTool(this.tool, p, {
	...
	App.prototype.pointerMove = function (p) {
		...
		if (this.drawing.mode === 'move') {
			...
			this.selectedObjects().forEach(function (obj) { global.LMPCBTracerTools.moveObject(obj, dx, dy); });
	...
	App.prototype.hitTest = function (p) {
		...
		this.state.objects.forEach(function (obj) {
			...
			var bounds = global.LMPCBTracerTools.getObjectBounds(obj);
	...
	App.prototype.duplicateSelected = function () {
		...
		this.selectedObjects().forEach(function (obj) {
			var copy = global.LMPCBTracerClone(obj);
			copy.id = global.LMPCBTracerTools.makeId();
			...
			if (componentId) {
				if (!componentMap[componentId]) {
					...
					if (component) {
						var componentCopy = global.LMPCBTracerClone(component);
			...
			global.LMPCBTracerTools.moveObject(copy, 1, 1);
	...
	App.prototype.copySelected = function () {
		this.clipboard = this.selectedObjects().map(function (obj) {
			return global.LMPCBTracerClone(obj);
		});
	};
	...
	App.prototype.pasteClipboard = function () {
		...
		this.clipboard.forEach(function (obj) {
			var copy = global.LMPCBTracerClone(obj);
			copy.id = global.LMPCBTracerTools.makeId();
			global.LMPCBTracerTools.moveObject(copy, 2, 2);
			this.state.objects.push(copy);
			ids.push(copy.id);
		}, this);
		this.selected = ids;
	...
	App.prototype.rotateSelected = function (degrees) {
		...
		this.history.push(this.state);
		this.selectedObjects().forEach(function (obj) {
			global.LMPCBTracerTools.rotateObject(obj, degrees);
		});
	...
	App.prototype.mirrorSelected = function () {
		...
		objects.forEach(function (obj) { global.LMPCBTracerTools.mirrorObject(obj, centerX); });
		this.refreshSelectedComponents();
		this.markDirty();
	};
	...
	App.prototype.alignObjects = function (mode) {
		...
		this.history.push(this.state);
		var items = objects.map(function (obj) { return { obj: obj, b: global.LMPCBTracerTools.getObjectBounds(obj) }; });
		...
		items.forEach(function (item) {
			...
			if (dx || dy) global.LMPCBTracerTools.moveObject(item.obj, dx, dy);
		});
		this.refreshSelectedComponents();
		this.markDirty();
	};
	...
	App.prototype.distributeObjects = function (axis) {
		...
		items.forEach(function (item, index) {
			...
			if (axis === 'x') global.LMPCBTracerTools.moveObject(item.obj, delta, 0);
			else global.LMPCBTracerTools.moveObject(item.obj, 0, delta);
		});
		this.refreshSelectedComponents();
		this.markDirty();
	};
	...
	App.prototype.showAlignMenu = function () {
		...
		var alignRow = el('div');
		alignRow.className = 'lm-pcb-align-row';
		[
			['align-left', 'Trái'],
		...
		var distRow = el('div');
		distRow.className = 'lm-pcb-align-row';
		...
		for (var row = 0; row < config.rows; row++) {
			for (var col = 0; col < config.cols; col++) {
				...
				allNew = allNew.concat(this.cloneSelectionForArray(function (obj) {
					global.LMPCBTracerTools.moveObject(obj, dx, dy);
				}));
			}
		}
	...
	App.prototype.createCircularArray = function (config) {
		...
		for (var i = 1; i < config.count; i++) {
			var angle = step * i;
			allNew = allNew.concat(this.cloneSelectionForArray(function (obj) {
				global.LMPCBTracerTools.rotateObjectAroundPoint(obj, config.cx, config.cy, angle);
			}));
		}
		if (allNew.length) this.selected = allNew;
		this.markDirty();
	...
	App.prototype.showSelectedProperties = function () {
		var panel = qs(this.root, '.lm-pcb-properties');
		if (!panel) return;
		panel.scrollIntoView({ block: 'nearest', inline: 'nearest' });
		panel.classList.add('lm-tablet-panel-flash');
		global.setTimeout(function () { panel.classList.remove('lm-tablet-panel-flash'); }, 700);
	};
	...
	App.prototype.save = function () {
		var self = this;
		if (!this.projectId) {
			return global.LMPCBTracerStorage.create({ name: this.state.project.name, code: this.state.project.code, width_mm: this.state.board.width_mm, height_mm: this.state.board.height_mm }).then(function (res) {
				self.projectId = res.id;
				return self.save();
			});
		}
		this.setSaveStatus('Đang lưu');
		return global.LMPCBTracerStorage.save(this.projectId, this.state).then(function (res) {
			self.state = res.data;
			self.dirty = false;
			self.setSaveStatus('Đã lưu');
			self.renderAll();
	...
	App.prototype.uploadImage = function (side, file) {
		...
		global.LMPCBTracerStorage.upload(file).then(function (res) {
	...
	App.prototype.boardResizeMove = function (totalDxMm, totalDyMm) {
		...
		if (incDx || incDy) {
			...
			this.state.objects.forEach(function (obj) {
				...
				obj.locked = false;
				global.LMPCBTracerTools.moveObject(obj, incDx, incDy);
	...
	App.prototype.exportGerber = function () {
		if (!global.LMPCBGerberExporter) {
			this.showMessage('Gerber exporter chưa được tải.');
			return;
		}
		try {
			var exporter = new global.LMPCBGerberExporter();
	...
	App.prototype.previewGerber = function () {
		...
		try {
			var result = new global.LMPCBGerberExporter().exportProject(this.state);
			var box = el('div');
	...
	App.prototype.runDrcCheck = function (kind) {
		...
		} else if (kind === 'unconnected' && global.LMPCBGeometryExporter) {
			warnings = new global.LMPCBGeometryExporter(this.state).export().warnings.filter(function (warning) {
		...
		if (target === 'component') {
			...
			if (key === 'x' || key === 'y') {
				...
				this.componentObjects(component.id).forEach(function (componentObj) {
					global.LMPCBTracerTools.moveObject(componentObj, dx, dy);
				});
				this.syncAnchoredTracks();
			}
	...
	App.prototype.showUserAccount = function () {
		var templatePanel = qs(this.root, '.lm-pcb-account-template [data-pcb-account-panel]');
	...
	document.addEventListener('DOMContentLoaded', function () {
		var root = document.getElementById('lm-pcb-tracer-app');
		if (root) {
			applyFrontendFullscreen(root);
			global.LMPCBTracerApp = new App(root);
			if (root.classList.contains('lm-pcb-tracer-fullscreen')) {
				global.setTimeout(function () { global.LMPCBTracerApp.canvas.resize(); }, 50);
			}
			root.dispatchEvent(new CustomEvent('datpcb:ready', { detail: { app: global.LMPCBTracerApp } }));
			if (global.LMPCBTabletMode && global.LMPCBTabletMode.bootstrap) {
				global.LMPCBTabletMode.bootstrap(global.LMPCBTracerApp);
			}
		}
	});
})(window, document);
