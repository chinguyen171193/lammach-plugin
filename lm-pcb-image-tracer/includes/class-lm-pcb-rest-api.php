...
class LM_PCB_REST_API {
	private $project;
	private $upload;
	private $ai;
	private $namespace = 'lm-pcb-tracer/v1';

	public function __construct( LM_PCB_Project $project, LM_PCB_Upload $upload, LM_PCB_AI $ai ) {
		...
		foreach ( $query->posts as $post ) {
			if ( current_user_can( 'edit_post', $post->ID ) ) {
				$items[] = array(
					...
					'code'     => get_post_meta( $post->ID, LM_PCB_TRACER_META_CODE, true ),
	...
	public function import_json( WP_REST_Request $request ) {
		...
		if ( ! is_array( $params ) || empty( $params['data'] ) || ! is_array( $params['data'] ) ) {
			return new WP_Error( 'lm_pcb_import_invalid', 'JSON không hợp lệ.', array( 'status' => 400 ) );
		}
		$data = wp_json_encode( $params['data'] );
		if ( strlen( $data ) > 2097152 ) {
			return new WP_Error( 'lm_pcb_import_large', 'JSON vượt quá 2 MB.', array( 'status' => 400 ) );
		}
		if ( isset( $params['data']['version'] ) && '1.0' !== (string) $params['data']['version'] ) {
			return new WP_Error( 'lm_pcb_import_version', 'Phiên bản JSON chưa được hỗ trợ.', array( 'status' => 400 ) );
		}
