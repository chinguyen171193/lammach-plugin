...
class LM_PCB_AI {
	const OPTION_API_KEY     = 'lm_pcb_tracer_openai_api_key';
	const OPTION_MODEL       = 'lm_pcb_tracer_openai_model';
	const OPTION_WEB_SEARCH  = 'lm_pcb_tracer_openai_web_search';
	const OPTION_MODEL_FIXED = 'lm_pcb_tracer_openai_model_upgraded';
	const OPTION_DEBUG       = 'lm_pcb_tracer_openai_debug';
	const OPTION_LAST_LOG    = 'lm_pcb_tracer_openai_last_log';
	...
	public function generate_component( $params ) {
		...
		if ( '' === trim( $prompt ) ) {
			return new WP_Error( 'lm_pcb_ai_empty_prompt', 'Vui long nhap ten linh kien hoac noi dung datasheet.', array( 'status' => 400 ) );
		}
		...
		if ( '' === $api_key ) {
			return new WP_Error( 'lm_pcb_ai_missing_key', 'Chua cau hinh OpenAI API key trong LM PCB Tracer - Cai dat. Cac linh kien/mach co san (LM2596, 7805, BC547, header, tu dien...) van dung duoc ma khong can API key.', array( 'status' => 400 ) );
		}
	...
	public function chat_message( $params ) {
		...
		if ( '' === trim( $message ) ) {
			return new WP_Error( 'lm_pcb_ai_empty_message', 'Vui long nhap noi dung.', array( 'status' => 400 ) );
		}
		...
		$api_key = $this->get_api_key();
		if ( '' === $api_key ) {
			return new WP_Error( 'lm_pcb_ai_missing_key', 'Khong nhan dien duoc yeu cau bang du lieu cuc bo. Can cau hinh OpenAI API key (LM PCB Tracer - Cai dat) de chat AI xu ly cac yeu cau khac (sua/xoa linh kien co san, linh kien la...).', array( 'status' => 400 ) );
		}
	...
	private function request_plan( $api_key, $body ) {
		...
		if ( 'lm_pcb_ai_api_rejected' === $code && isset( $body['tools'] ) ) {
		...
		if ( in_array( $code, array( 'lm_pcb_ai_api_rejected', 'lm_pcb_ai_invalid_json' ), true ) && isset( $body['tools'] ) ) {
			unset( $body['tools'], $body['tool_choice'] );
			$plan = $this->post_and_decode( $api_key, $body );
		}

		if ( is_wp_error( $plan ) && 'lm_pcb_ai_api_rejected' === $plan->get_error_code() ) {
			return new WP_Error( 'lm_pcb_ai_api_error', $plan->get_error_message(), array( 'status' => 502 ) );
		}
		return $plan;
	}
	...
	private function post_openai( $api_key, $body ) {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error( 'lm_pcb_ai_request_failed', $response->get_error_message(), array( 'status' => 502 ) );
		}
		...
		if ( $code < 200 || $code >= 300 ) {
			...
			$code_id = 400 === (int) $code ? 'lm_pcb_ai_api_rejected' : 'lm_pcb_ai_api_error';
			return new WP_Error( $code_id, sanitize_text_field( $message ), array( 'status' => 502 ) );
		}
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'lm_pcb_ai_invalid_json', 'OpenAI khong tra ve JSON hop le.', array( 'status' => 502 ) );
		}
		return $data;
	}
	...
	private function decode_plan( $data ) {
		...
		if ( '' !== $refusal ) {
			return new WP_Error( 'lm_pcb_ai_refused', sanitize_text_field( $refusal ), array( 'status' => 502 ) );
		}
		...
		if ( $incomplete || '' === trim( (string) $text ) ) {
			...
			return new WP_Error(
				'lm_pcb_ai_truncated',
				'AI dung het han muc token cho phan suy luan truoc khi kip tra ket qua (luot goi nay van bi tinh tien). Hay tat "Tra cuu Internet" trong LM PCB Tracer - OpenAI, hoac chia yeu cau thanh tung phan nho hon.',
				array( 'status' => 502 )
			);
		}
		return new WP_Error( 'lm_pcb_ai_invalid_json', 'OpenAI khong tra ve JSON hop le.', array( 'status' => 502 ) );
	...
	private function build_datasheet_file_input( $file ) {
		...
		if ( ! empty( $file['error'] ) ) {
			return new WP_Error( 'lm_pcb_ai_upload_error', 'Khong doc duoc file datasheet.', array( 'status' => 400 ) );
		}
		$name = isset( $file['name'] ) ? sanitize_file_name( $file['name'] ) : 'datasheet.pdf';
		$size = isset( $file['size'] ) ? (int) $file['size'] : 0;
		if ( $size <= 0 || $size > 8 * 1024 * 1024 ) {
			return new WP_Error( 'lm_pcb_ai_file_size', 'Datasheet PDF phai nho hon 8 MB.', array( 'status' => 400 ) );
		}
		if ( 'pdf' !== strtolower( pathinfo( $name, PATHINFO_EXTENSION ) ) ) {
			return new WP_Error( 'lm_pcb_ai_file_type', 'Chi ho tro datasheet PDF.', array( 'status' => 400 ) );
		}
		$bytes = file_get_contents( $file['tmp_name'] );
		if ( false === $bytes ) {
			return new WP_Error( 'lm_pcb_ai_file_read', 'Khong doc duoc file datasheet.', array( 'status' => 400 ) );
		}
		return array(
			'type'      => 'input_file',
			'filename'  => $name,
	...
	private function lcsc_footprint( $mpn, $ref ) {
		$mpn = trim( (string) $mpn );
		if ( '' === $mpn || ! class_exists( 'LM_PCB_LCSC' ) ) {
			return null;
		}
		if ( null === $this->lcsc ) {
			$this->lcsc = new LM_PCB_LCSC();
		}
...
' . $this->circuit_design_instructions() . "\n" . $this->footprint_geometry_instructions() . "\n" . $this->research_instructions();
	 ...
	 *
	 * Lm theo dung cach EasyEDA ve: hai hang NGANG, chieu dai than chay theo truc
