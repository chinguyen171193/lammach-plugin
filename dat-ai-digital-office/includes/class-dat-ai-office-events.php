...
class DAT_AI_Office_Events {
	...
	public function __construct() { add_action( 'dat_ai_office_event', array( $this, 'receive' ), 10, 1 ); }

	public function receive( $event ) {
		$event = is_array( $event ) ? $event : array();
		$type = sanitize_key( $event['type'] ?? '' );
		$fallback = $this->map[ $type ] ?? array( 'ai_center', 'DAT Supervisor AI', 'Sự kiện mới' );
		$record = array(
