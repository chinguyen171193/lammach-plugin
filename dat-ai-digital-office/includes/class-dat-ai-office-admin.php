...
class DAT_AI_Office_Admin {
	...
	public function animation_add() { if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'Không đủ quyền.' ); } check_admin_referer( 'dat_ai_office_animation_add' ); $result = DAT_AI_Office_Animations::add( sanitize_key( $_POST['employee'] ?? '' ), sanitize_key( $_POST['action_key'] ?? '' ), absint( $_POST['attachment_id'] ?? 0 ) ); $this->animation_redirect( is_wp_error( $result ) ? $result->get_error_message() : 'Đã lưu chuyển động.' ); }
	public function animation_remove() { if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'Không đủ quyền.' ); } check_admin_referer( 'dat_ai_office_animation_remove' ); $result = DAT_AI_Office_Animations::remove( sanitize_key( $_POST['employee'] ?? '' ), sanitize_key( $_POST['action_key'] ?? '' ), absint( $_POST['variant_index'] ?? 0 ) ); $this->animation_redirect( is_wp_error( $result ) ? $result->get_error_message() : 'Đã xóa liên kết chuyển động.' ); }
