...
class DAT_AI_Office_Admin {
	...
	public function __construct() { add_action( 'admin_menu', array( $this, 'menu' ) ); add_action( 'admin_init', array( $this, 'settings' ) ); add_action( 'admin_post_dat_ai_office_reset', array( $this, 'reset' ) ); add_action( 'admin_post_dat_ai_office_animation_add', array( $this, 'animation_add' ) ); add_action( 'admin_post_dat_ai_office_animation_remove', array( $this, 'animation_remove' ) ); add_action( 'admin_post_dat_ai_office_character_model_save', array( $this, 'character_model_save' ) ); }
	...
	public function character_model_save() { if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'Không đủ quyền.' ); } check_admin_referer( 'dat_ai_office_character_model_save' ); $result = DAT_AI_Office_Characters::set_model( sanitize_key( $_POST['employee'] ?? '' ), absint( $_POST['attachment_id'] ?? 0 ) ); wp_safe_redirect( add_query_arg( array( 'page' => 'dat-ai-office-characters', 'character_notice' => rawurlencode( is_wp_error( $result ) ? $result->get_error_message() : 'Đã lưu model nhân vật.' ) ), admin_url( 'admin.php' ) ) ); exit; }
