	...
	class OfficeUI {
		...
		showAgent(agent) {
			...
			if (sprite && global.DAT_AgentSpritePlayer) {
				this.spritePlayer = global.DAT_AgentSpritePlayer.create(sprite, { agentId: data.id, spriteId: data.sprite || '_placeholder', state: data.status, assetBase: (global.DAT_AI_OFFICE || {}).agentAssetsUrl || '', scale: 1 });
			}
