	...
	class OfficeUI {
		...
		action(action) {
			...
			if (action === 'sound') {
				...
				this.bus.log({ actor: 'DAT Supervisor AI', department: 'ai_center', message: localStorage.getItem(key) === '1' ? 'Âm thanh đã bật.' : 'Âm thanh đã tắt.' });
			}
		}
		...
		renderMetrics() {
			...
			this.metrics.innerHTML = [['Nhân viên online', humans], ['AI Agent', ai], ['Đang xử lý', active], ['Hoàn thành', this.engine.completed], ['Hiệu suất', this.engine.efficiency + '%'], ['Tiết kiệm', this.engine.saved + ' phút']].map(metric => `<div class="dat-ai-office__metric"><b>${metric[1]}</b><span>${metric[0]}</span></div>`).join('');
		}
		...
		renderLogs() {
			...
			this.logList.innerHTML = this.bus.logs.filter(log => value === 'all' || log.department === value || (value === 'ai' && /AI/.test(log.actor))).slice(0, 32).map(log => `<article class="dat-ai-office__entry" data-level="${this.escape(log.level)}"><time>${this.time(log.created_at)} — ${this.escape(log.actor)}</time>${this.escape(log.message)}</article>`).join('');
		}
		...
		showAgent(agent) {
			...
			this.card.innerHTML = `<button class="dat-ai-office__close">×</button><div class="dat-agent-sprite" data-agent-sprite data-agent-id="${this.escape(data.id)}" data-sprite-id="${this.escape(data.sprite || '_placeholder')}" data-agent-state="${this.escape(data.status)}"><div class="dat-agent-sprite__surface" data-agent-sprite-surface></div><div class="dat-agent-sprite__fallback" aria-hidden="true"></div></div><h3>${this.escape(data.name)}</h3><p>${this.escape(data.role)} · ${this.escape(data.department)}</p><div class="dat-ai-office__row"><span>Trạng thái</span><b data-agent-status>${this.escape(data.status)}</b></div><div class="dat-ai-office__row"><span>Nhiệm vụ</span><b data-agent-task>${this.escape(data.task || 'Đang chờ')}</b></div><div class="dat-ai-office__row"><span>Tiến độ</span><b data-agent-progress-value>${Math.round(data.progress || 0)}%</b></div><div class="dat-ai-office__agent-progress"><i data-agent-progress-bar style="width:${Math.max(0, Math.min(100, Number(data.progress) || 0))}%"></i></div><p>Lịch sử: ${this.escape((data.history || []).join(' · '))}</p><button data-follow>Camera follow</button> <button data-move>Chuyển phòng</button>`;
			const sprite = this.card.querySelector('[data-agent-sprite]');
			if (sprite && global.DAT_AgentSpritePlayer) {
				this.spritePlayer = global.DAT_AgentSpritePlayer.create(sprite, { agentId: data.id, spriteId: data.sprite || '_placeholder', state: data.status, assetBase: (global.DAT_AI_OFFICE || {}).agentAssetsUrl || '', assetVersion: (global.DAT_AI_OFFICE || {}).agentAssetsVersion || '', scale: 1 });
			}
			this.card.querySelector('.dat-ai-office__close').onclick = () => this.closeAgent();
		...
		async submitTask(event) {
			...
			try {
				const response = await fetch(global.DAT_AI_OFFICE.restUrl + 'tasks', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': global.DAT_AI_OFFICE.nonce }, body: JSON.stringify(data) });
				...
				this.bus.log({ actor: 'DAT Supervisor AI', department: data.department, level: 'success', message: 'Đã giao nhiệm vụ: ' + data.title });
				this.hideTask();
