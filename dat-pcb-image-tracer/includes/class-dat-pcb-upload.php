...
class DAT_PCB_Upload {
	...
	public function handle_upload( $file_key ) {
		if ( ! is_user_logged_in() || ! current_user_can( 'upload_files' ) ) {
			return new WP_Error( 'dat_pcb_upload_forbidden', 'Không đủ quyền upload.', array( 'status' => 403 ) );
		}
		if ( empty( $_FILES[ $file_key ] ) || ! is_array( $_FILES[ $file_key ] ) ) {
			return new WP_Error( 'dat_pcb_upload_missing', 'Thiếu file ảnh.', array( 'status' => 400 ) );
		}
		$file = $_FILES[ $file_key ];
		if ( ! empty( $file['size'] ) && (int) $file['size'] > self::MAX_IMAGE_BYTES ) {
			return new WP_Error( 'dat_pcb_upload_too_large', 'Ảnh vượt quá giới hạn 15 MB.', array( 'status' => 400 ) );
		}
		...
		if ( empty( $check['type'] ) || ! in_array( $check['type'], array( 'image/jpeg', 'image/png', 'image/webp' ), true ) ) {
			return new WP_Error( 'dat_pcb_upload_type', 'Chỉ chấp nhận JPG, JPEG, PNG, WEBP.', array( 'status' => 400 ) );
		}
